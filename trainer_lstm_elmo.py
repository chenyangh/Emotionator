...
import pandas as pd
from models.sa_lstm_classifier_elmo import AttentionLSTMClassifier
...
def train(X_train, y_train, X_dev, y_dev, X_test, y_test):
    ...
    model = AttentionLSTMClassifier(EMBEDDING_DIM, HIDDEN_DIM, vocab_size,
                                    num_labels, BATCH_SIZE, att_mode=opt.attention, use_glove=USE_GLOVE)
...
def main():
    ...
    for k in range(NUM_FOLDS):
        ...
        with open(f'lstm_elmo_{opt.dataset}_tokenizer.pkl', 'bw') as f:
            pkl.dump(tokenizer, f)

        VOCAB_SIZE = tokenizer.get_vocab_size()
        if USE_GLOVE:
            print('Building GloVe embedding')
            tokenizer.build_embedding(opt.embpath, save_pkl=SAVE_PKL)
...
def interactive_inference(model_token=''):
    ...
    model = AttentionLSTMClassifier(EMBEDDING_DIM, HIDDEN_DIM, tokenizer.get_vocab_size(),
                                    NUM_EMO, BATCH_SIZE, att_mode=opt.attention)
    # multi-GPU
    # model = nn.DataParallel(model)
    ...
    while True:
        ...
        y_pred = model(seq.cuda(), seq_len, elmo_emb)
